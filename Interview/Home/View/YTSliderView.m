...
- (void)configView {
    ...
    self.thumbView.frame = CGRectMake(0, 0, 10, 40);
...
- (UIView *)thumbView {
    if(!_thumbView) {
        _thumbView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, 15, 40)];
        _thumbView.backgroundColor = [UIColor clearColor];
        UIImageView *imgV =[[UIImageView alloc]initWithFrame:_thumbView.bounds];
        imgV.image = [UIImage imageNamed:@"滑块"];
        [_thumbView addSubview:imgV];
    }
    return _thumbView;
}
