...
-(void)setIsShowTitle:(BOOL)isShowTitle{
    ...
    if (_isShowTitle == YES) {
        ...
        self.sliderValueLabel.textAlignment = NSTextAlignmentLeft;
        self.sliderValueLabel.font = [UIFont systemFontOfSize:13];
        self.sliderValueLabel.backgroundColor = [[UIColor whiteColor] colorWithAlphaComponent:1];
...
//        self.sliderValueLabel.text = [NSString stringWithFormat:@"%.f%%", self.value];
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
             UIImageView *slideImage = (UIImageView *)[self.subviews objectAtIndex:2];
            
            //滑块的值
            [self.sliderValueLabel mas_makeConstraints:^(MASConstraintMaker *make) {
                if (self.titleStyle == RightTitleStyle) {
                    make.leading.mas_equalTo(slideImage.mas_trailing).offset(10);
                }else if(self.titleStyle == TopTitleStyle){
                    make.bottom.mas_equalTo(slideImage.mas_top).offset(-30);
                }else{
                    
                }
                make.width.mas_equalTo(45);
                make.centerX.equalTo(slideImage.mas_centerX).offset(10);
            }];
        });
    }
}
...
- (void)masLayoutSubviews{
    ...
    CGPoint center = CGPointMake(20/2, -p);
    self.thumbImageView.center = center;
//    NSLog(@"thumbImageView---%@",NSStringFromCGPoint(center));
    ...
    if (recognizer.state == UIGestureRecognizerStateChanged) {
        ...
        if (self.thumbImageView.frame.origin.y + self.thumbImageView.frame.size.height <= self.frame.size.height &&
            ...
            if (self.touchSliderValueChange) {
                self.touchSliderValueChange(value,NO,self.tag);
            }
        }
...
//        NSLog(@"subviews---%@",self.sliderValueLabel.subviews);
        ...
        if (self.touchSliderValueChange) {
            self.touchSliderValueChange(self.value,YES,self.tag);
        }
    }
}
...
- (UIImageView *)thumbImageView{
    if (!_thumbImageView) {
        ...
        _thumbImageView.image = [UIImage imageNamed:@"圆形"];
    }
