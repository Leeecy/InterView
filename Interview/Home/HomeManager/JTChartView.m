...
//    [self createObservers];
    ...
    [self drawGraphWithValues:values inView:self.subView minY:minYDefault maxY:maxYDefault topPadding:topPaddingDefault curveColor:curveColorDefault curveWidth:curveWidthDefault topGradientColor:topGradientColorDefault bottomGradientColor:bottomGradientColorDefault];
    
    
    [self createSliderWithArr:values];
...
-(void)createSliderWithArr:(NSArray *)values{
    
    CGFloat  space = self.bounds.size.width/4.0;
   
    for (NSInteger i = 0; i<values.count; i++) {
        // UISlider
        UISlider *flySlider = [[UISlider alloc] initWithFrame:CGRectMake((i-1)*space + 5, 190 , 200, 5)];
        flySlider.maximumValue                  = 200.f;
        flySlider.minimumValue                  = 0.f;
        flySlider.value                         = [values[i] floatValue];
        flySlider.maximumTrackTintColor         = [UIColor clearColor];
        flySlider.minimumTrackTintColor         = [UIColor clearColor];
        //实现偏转
        flySlider.transform                     = CGAffineTransformMakeRotation(-M_PI_2);
        flySlider.tag = i+1;
        
        [flySlider addTarget:self action:@selector(sliderValueChanged:) forControlEvents:UIControlEventValueChanged];
        
        [self addSubview:flySlider];
    }
