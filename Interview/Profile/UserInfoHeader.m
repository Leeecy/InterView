...
- (void)initData:(User *)user {
    ...
    [_avatar sd_setImageWithURL:[NSURL URLWithString:user.avatar_medium.url_list.firstObject] completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
    ...
    [_slideBar setLabels:@[[@"作品" stringByAppendingString:[NSString stringWithFormat:@"%ld", (long)user.aweme_count]],[@"喜欢" stringByAppendingString:[NSString stringWithFormat:@"%ld", (long)user.favoriting_count]],[@"收藏" stringByAppendingString:[NSString stringWithFormat:@"%ld", (long)user.aweme_count]]] tabIndex:0];
}
...
- (void)overScrollAction:(CGFloat) offsetY {
    ...
    _topBackground.transform = CGAffineTransformConcat(CGAffineTransformMakeScale(scaleRatio + 1.0f, scaleRatio + 1.0f), CGAffineTransformMakeTranslation(0, -overScaleHeight));//缩放 平移 然后合并
}
